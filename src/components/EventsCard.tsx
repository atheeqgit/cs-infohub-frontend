...
//         <div className="absolute inset-0 bg-white flex flex-col px-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl text-left p-4 py-6 overflow-y-auto">
...
const EventsCard: React.FC<EventCardProps> = ({ event }) => {
  return (
    <div
      ...
      className={`relative mx-auto mb-10 w-full md:w-auto cursor-pointer group focus:outline-none ${
        event.imgType === "Portrait"
          ? "col-span-2 md:col-span-3"
          : "col-span-2 md:col-span-6"
      }`}
    >
      ...
      <div className="relative w-full h-full border border-gray-500 overflow-hidden rounded-2xl shadow-2xl transition-all duration-300">
        ...
        {/* Hidden Card - Appears on Hover or Focus */}
        <div className="absolute inset-0 bg-white flex flex-col px-4 opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-300 rounded-2xl text-left p-4 py-6 overflow-y-auto pb-10">
          <div className="flex flex-row justify-between items-center">
            <p className="text-lg font-semibold">{event.eventType}</p>
            <h3 className="text-sm font-semibold">{event.date}</h3>
          </div>
      ...
      <div className="w-[85%] border border-solid p-2 md:p-4 text-center rounded-xl text-red-800 capitalize font-semibold backdrop-blur-md absolute bg-[#ffffff94] bottom-[-6%] left-[6%] transition-all duration-300 group-hover:shadow-[7px_7px_0px_0px_rgba(170,17,23,1)] group-focus:shadow-[7px_7px_0px_0px_rgba(170,17,23,1)]">
        {event.eventTitle}
      </div>
    </div>
  );
